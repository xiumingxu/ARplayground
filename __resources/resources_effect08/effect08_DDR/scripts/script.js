...
const types = [ null, 'lookLeft', 'openMouth', 'blink', 'lookRight' ];

// Handle gestures from patch editor
Patches.getPulseValue('blinkGesture').subscribe(function (e) {
	gestureHandler('blink');
});
Patches.getPulseValue('lookLeftGesture').subscribe(function (e) {
	gestureHandler('lookLeft');
});
Patches.getPulseValue('lookRightGesture').subscribe(function (e) {
	gestureHandler('lookRight');
});
Patches.getPulseValue('openMouthGesture').subscribe(function (e) {
	gestureHandler('openMouth');
});
...
const offsets = drivers.map(function (driver) {
...
function incrementScore (amount) {
	...
	score.toString().split('').reverse().forEach(function (digit, i) {
		tempArr[i] = digit;
	});
...
function gestureHandler (type) {
	...
	if (!gameStarted) {
		...
		if (type == currMove) {
			...
			Time.setTimeout(function () {
				scoreZone.material.diffuse = scoreTex[1];
			}, duration / 15);
...
userScope
	...
	.then(function (data) {
		...
		var tempArr = [ 0, 0, 0, 0, 0 ];
		highScore.toString().split('').reverse().forEach(function (digit, i) {
			tempArr[i] = digit;
		});
	...
	.catch(function (error) {
		Diagnostics.log('unable to load high score data');
	});

function saveHighScore () {
	// Indicate high score
	Time.setTimeout(function () {
		playAudio('playCongratsAudio');
	}, 1200);
	...
	userScope
		.set('DDR_high_score', { score: score })
		.then(function () {
			Diagnostics.log('High score saved');
		})
		.catch(function (error) {
